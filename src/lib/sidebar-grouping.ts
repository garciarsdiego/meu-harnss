...
export interface SidebarItem {
  type: "pinned" | "branch" | "folder" | "session";
  /** Human-readable label for section headers. */
  label: string;
  /** Sort key — newest lastMessageAt within this group. */
  sortTimestamp: number;
  /** Single session (for type === "session"). */
  session?: ChatSession;
  /** Folder metadata (for type === "folder"). */
  folder?: ChatFolder;
  /** Sessions within this group (folder, branch, pinned). */
  sessions: ChatSession[];
  /** Nested items within a branch section (folders + ungrouped sessions). */
  children?: SidebarItem[];
  /** Branch name (for type === "branch"). */
  branchName?: string;
}
...
export function buildSidebarGroups(
  ...
  if (pinnedSessions.length > 0 || pinnedFolders.length > 0) {
    ...
    const pinnedFolderItems: SidebarItem[] = pinnedFolders.map((folder) => {
